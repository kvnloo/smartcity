...
class OriginFrame(ContractModel):
    ...
    Ring fills and the GitHub Pages camera use `ring_center_*` (Naperville
    downtown). That is not the mesh origin in city.json.
    """
    ...
    unreal_uu_per_metre: float = Field(
        description=(
            "Multiply x_m/y_m by this to reach default Unreal world units (centimetres). "
            "Use 1.0 if World Settings are metres."
        )
    )
