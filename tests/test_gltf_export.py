...
def test_hero_clip_is_downtown_not_osm_centroid():
    meta = load_city_meta(FIXTURE)
    origin = resolve_origin_lonlat(meta)
    utm = resolve_origin_utm(meta)
    cx, cy = downtown_xy_in_mesh(origin, utm)
    assert cx < -3000
    assert abs(cy) < 800
    assert hero_keep(0.0, 0.0, 1500.0, cx, cy) is False
    assert hero_keep(cx, cy, 1500.0, cx, cy) is True
    assert DOWNTOWN_LONLAT == (-88.147, 41.75)
...
def test_sidecar_points_unreal_at_glb_and_city_origin(tmp_path: Path):
    ...
    assert payload["unreal"]["origin_lonlat"] == [-88.147, 41.75]
    assert payload["unreal"]["cesium_georeference_origin_lonlat"] == [-88.147, 41.75]
    assert payload["unreal"]["place_at"][0] > 300_000
    assert payload["unreal"]["place_at"][1] > 30_000
    assert payload["hero_clip_center_mesh_m"][0] < -3000
