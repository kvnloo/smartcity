...
| OSM / Blender glTF (city.json XY + downtown place_at) | Google Photorealistic 3D Tiles |
...
Default Unreal world units are **centimetres**. `origin.unreal_uu_per_metre` is `100`. Unreal `(0,0,0)` is downtown (`ring_center_*`), not the mesh origin. Spawn with

```
FVector((x_m - downtown_x_m) * 100, (y_m - downtown_y_m) * 100, z_m * 100)
```

unless World Settings are metres (then use `1` instead of `100`). Parenting to the glTF actor that already sits at sidecar `unreal.place_at` is the other legal option: then local `FVector(x_m * 100, y_m * 100, z_m * 100)` is correct. glTF from Blender is metres; the importer must use the same scale.
...
| --- | --- | --- | --- |
| **Mesh origin** | `twin.origin.lon/lat` and `GET /city` `origin_lonlat` | **-88.106604, 41.75421** | Local metres, SUMO edges, `/ws` `x_m`/`y_m` |
| **Downtown in mesh XY** | `twin.origin.downtown_x_m` / `downtown_y_m` | (derived) | Typically **−3364.64, −423.49**. glTF `place_at` = `(-downtown_x_m, -downtown_y_m) * 100` UU |
| **Unreal / Cesium world origin** | `twin.origin.ring_center_lon/lat`, `Georef.json` | **-88.147, 41.75** | Unreal `(0,0,0)`. Fidelity rings and the Pages camera |
...
uu_x     = (x_m - origin.downtown_x_m) * origin.unreal_uu_per_metre
uu_y     = (y_m - origin.downtown_y_m) * origin.unreal_uu_per_metre
```
...
2. Set Cesium / World Partition origin to `origin.ring_center_lon/lat` (downtown), CRS EPSG:32616. Place the city.json glTF at `(-downtown_x_m * 100, -downtown_y_m * 100, 0)` UU (sidecar `unreal.place_at`).
3. Spawn at most a few hundred vehicle pawns. Pool them. Cull at 80–120 m. `kind` selects the livery.
4. Connect `ws://127.0.0.1:43147/ws`. On each `type=snapshot` frame, update pooled pawns by `vehicles[].id` using `x_m`, `y_m`, `heading` (yaw = heading if X=east, Y=north, Z=up). World uu = `(x_m - downtown_x_m) * 100`.
